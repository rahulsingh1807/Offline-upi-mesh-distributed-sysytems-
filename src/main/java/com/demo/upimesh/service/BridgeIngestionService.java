...
public class BridgeIngestionService {
    ...
    public IngestResult ingest(MeshPacket packet, String bridgeNodeId, int hopCount) {
        try {
            ...
            // ---- Settle ----
            // BUG FIX (frontend not updating - root cause #2 continued):
            // Previously this always called IngestResult.settled(...) regardless
            // of whether SettlementService actually moved money. Now we branch
            // on SettleResult.rejected() so the dashboard sees the true outcome.
            SettlementService.SettleResult result =
                    settlement.settle(instruction, packetHash, bridgeNodeId, hopCount);

            if (result.rejected()) {
                return IngestResult.rejected(packetHash, result.transaction().getId());
            }
            return IngestResult.settled(packetHash, result.transaction());
    ...
    public record IngestResult(String outcome, String packetHash, String reason, Long transactionId) {
        ...
        // BUG FIX: new factory method so a real REJECTED outcome (insufficient
        // funds) is distinguishable from SETTLED in the API response.
        public static IngestResult rejected(String hash, Long txId) {
            return new IngestResult("REJECTED", hash, "insufficient_funds", txId);
        }
