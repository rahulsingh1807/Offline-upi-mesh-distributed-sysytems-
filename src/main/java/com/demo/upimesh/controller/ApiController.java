...
public class ApiController {
    ...
    // BUG FIX (frontend not updating - root cause #3):
    // Previously this returned "packetIds": ["abcd1234", ...] — just truncated
    // IDs with no TTL info. The dashboard's TTL dots were hardcoded/fake because
    // there was no real per-packet TTL in the API response at all.
    // Now each device returns "packets": [{id, ttl}, ...] so the UI can render
    // accurate, live TTL dots that actually change as gossip rounds run.
    @GetMapping("/mesh/state")
    public Map<String, Object> meshState() {
        List<Map<String, Object>> deviceData = new ArrayList<>();
        for (VirtualDevice d : mesh.getDevices()) {
            List<Map<String, Object>> packets = d.getHeldPackets().stream()
                    .map(p -> Map.<String, Object>of(
                            "id", p.getPacketId().substring(0, 8),
                            "ttl", p.getTtl()
                    ))
                    .toList();
     ...
     * They all upload everything they hold to /api/bridge/ingest in parallel —
     * this is the concurrent duplicate case that exercises idempotency.
     */
